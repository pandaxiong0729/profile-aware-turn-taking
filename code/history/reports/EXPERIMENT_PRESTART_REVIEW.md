...
- 实际模型音频验收：通过。500 个 mono clip 已逐条从 16 个原始 WAV 的
  `[t-30s,t)` 时间窗重新计算并比较，500/500 完全一致，未来音频为 0。
- 科学标签验收：尚未通过。为节省时间，先复核预先固定的低风险 50 条；
  只有模型门禁通过后才继续复核完整 500 条。当前两者仍是自动弱标签。
...
| 固定低风险 50 条 pilot | `data/processed/sbcsae_mvp_v2/prompt_pilot_lowrisk_50.jsonl` |
| 50 条第一阶段复核页面 | `artifacts/mllm-prompt-baseline/qwen2.5-omni-3b/onset-balanced-lowrisk-50-review-required/review.html` |
...
这里不是三个模型，而是一个模型及其运行方式：

- `Qwen2.5-Omni-3B`：实际被评测的一个音频多模态模型；
- `Q4_K_M`：主模型权重的压缩格式，不是第二个模型；
- `llama.cpp`：加载和运行权重的推理程序，不是第三个模型；
- `mmproj-Q8_0`：同一个多模态模型用于把音频接入主模型的 projector 文件。

本机主权重约 2.10 GB，音频 projector 约 1.54 GB。它们共同组成这一次
Qwen2.5-Omni-3B 音频推理，不代表使用了两个独立 checkpoint。
...
| 三条件非 profile 字段变化 | 0 |
| 500 个模型音频与原始时间窗逐样本重建不一致 | 0 |

第一阶段 50 条另行审计：五类各 10 条、覆盖 16 会话、每类覆盖 10 个不同
会话、同会话最小边界间隔 9.24 秒，非词汇/环境/BC-I 边界自动风险均为 0；
150 个 paired requests 输入审计通过。它们仍是 0/50 人工复核，因此尚未开跑。
...
1. 在低风险 50 条页面完成标签复核，`U` 样本必须重新裁决。
2. 导出 JSON，生成 `reviewed_pilot_50.jsonl`，重新准备请求并运行 50 条协议门禁。
3. 门禁通过后才运行 50 样本/150 请求；同时做 hidden 原音频/静音对照。
4. 检查输出有效率、hidden 是否塌缩、是否对音频敏感；失败则停止，不再人工复核其余 450 条。
5. 若 50 条通过，再打开完整 500 条页面，并导入前面 50 条的复核 JSON，避免重复标注。
6. 完成剩余 450 条、导出完整 JSON，生成固定 `reviewed_500.jsonl`。
7. 重新运行 500 条 preflight 和完整协议门禁；通过后才跑 1,500 个请求。
8. 生成 metrics、混淆矩阵、配对变化、bootstrap CI、延迟和错误案例。
...
| 锁定协议 | `configs/mllm_prompt_pilot_locked.json` |
| 协议漂移/开跑门禁 | `scripts/verify_prompt_protocol.py` |

50 条和 500 条分别由 `mllm_prompt_pilot50_locked.json` 与
`mllm_prompt_pilot_locked.json` 锁定。当前 50 条协议应显示
`protocol_consistent=true`、`ready_for_50_sample_pilot=false`；唯一科学门禁是
50 条仍未完成人工复核，因此没有启动推理。

正式开跑前必须使用带 readiness 门禁的命令；只做协议一致性检查不够：

```powershell
.\.venv\Scripts\python.exe code/scripts/verify_prompt_protocol.py `
  --protocol code/configs/mllm_prompt_pilot50_locked.json --require-ready
```

当前该命令应被门禁拒绝；只有人工复核完成、重新生成 reviewed manifest 和
请求后，它才允许进入模型运行阶段。
