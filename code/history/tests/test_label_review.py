...
def test_build_and_apply_review(tmp_path: Path) -> None:
    ...
    page = (run / "review.html").read_text(encoding="utf-8")
    assert "SBCSAE 1-event label review" in page
    assert "sbcsae-review-v2-" in page
    assert "导入已有复核 JSON" in page
    assert "function importReviews" in page
