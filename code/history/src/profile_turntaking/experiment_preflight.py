...
def audit_prompt_pilot_data(
    *,
    ...
    gold_quality: dict[str, set[bool]] = defaultdict(set)
    for row in gold_rows:
        gold_targets[str(row["sample_id"])].add(str(row["target"]))
        gold_quality[str(row["sample_id"])].add(bool(row.get("gold_label", False)))
    mixed_quality = sum(len(gold_quality[sample_id]) != 1 for sample_id in selected_ids)
    if mixed_quality:
        errors.append(f"{mixed_quality} samples mix reviewed and weak target flags")
    reviewed_ids = {
        sample_id
        for sample_id in selected_ids
        if gold_quality.get(sample_id) == {True}
    }
    target_manifest_mismatches = sum(
        ...
        and sample_id not in reviewed_ids
    )
    if target_manifest_mismatches:
        errors.append(
            f"{target_manifest_mismatches} unreviewed request targets differ from weak manifest labels"
        )
    human_label_changes = sum(
        len(gold_targets[sample_id]) == 1
        and next(iter(gold_targets[sample_id])) != event_by_id[sample_id]["label"]
        for sample_id in reviewed_ids
        if sample_id in event_by_id
    )
    ...
    effective_target_counts = Counter(
        next(iter(gold_targets[sample_id]))
        for sample_id in selected_ids
        if len(gold_targets[sample_id]) == 1
    )
    ...
    prompt_pilot_ready = (
        ...
        and weak == 0
        and len(selected) == expected_samples
    )
    report = {
        ...
        "selected_set": {
            ...
            "weak_labels": weak,
            "human_label_changes_from_weak": human_label_changes,
            "effective_target_class_counts": {
                label: effective_target_counts[label] for label in LABELS
            },
